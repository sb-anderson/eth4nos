...
def main():
    ...
    for accountCnt in range(ACCOUNT_NUM):

        targetAddr = Web3.toChecksumAddress(hex((accountCnt+3)*2)[2:].zfill(40))
        addresses = list()
        addresses.append(targetAddr)
        sendRestoreTx(blockNum, addresses)
        print("\n")
        # if (accountCnt % TX_PER_BLOCK == TX_PER_BLOCK-1):
        #     blockNum += 1
...
def sendRestoreTx(currentBlock, addresses):
    ...
    for r, address in enumerate(addresses):
        ...
        # for pp in proofs:
        #     print("	isVoid: ", pp['IsVoid'], "	/ isBloom: ", pp['isBloom'])
        #     # print("proof: ", pp)
